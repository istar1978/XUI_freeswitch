 ...
 */
Blockly.JSON.ORDER_ATOMIC = 0;          // literals
// The next level was not explicit in documentation and inferred by Ellen.
Blockly.JSON.ORDER_HIGH = 1;            // Function calls, tables[]
Blockly.JSON.ORDER_EXPONENTIATION = 2;  // ^
Blockly.JSON.ORDER_UNARY = 3;           // not # - ~
Blockly.JSON.ORDER_MULTIPLICATIVE = 4;  // * / %
Blockly.JSON.ORDER_ADDITIVE = 5;        // + -
Blockly.JSON.ORDER_CONCATENATION = 6;   // ..
Blockly.JSON.ORDER_RELATIONAL = 7;      // < > <=  >= ~= ==
Blockly.JSON.ORDER_AND = 8;             // and
Blockly.JSON.ORDER_OR = 9;              // or
  ...
  if (!block.outputConnection || !block.outputConnection.targetConnection) {
    ...
    for (var i = 0; i < block.inputList.length; i++) {
      if (block.inputList[i].type == Blockly.INPUT_VALUE) {
        ...
        if (childBlock) {
          comment = Blockly.JSON.allNestedComments(childBlock);
          if (comment) {
            commentCode += Blockly.JSON.prefixLines(comment, '-- ');
          }
        }
